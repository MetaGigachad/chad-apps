...
func (h *Handler) Handle(ctx context.Context, r slog.Record) error {
    ...
    source := ""
    if attrs[slog.SourceKey] != nil {
        sourceData := attrs[slog.SourceKey].(map[string]interface{})
        source = colorize(darkGray, fmt.Sprintf("[%s:%s:%.0f]", sourceData["file"].(string), sourceData["function"].(string), sourceData["line"].(float64)))
...
func NewHandler(opts *slog.HandlerOptions) *Handler {
	return New(opts, WithDestinationWriter(os.Stderr), WithColor())
}
